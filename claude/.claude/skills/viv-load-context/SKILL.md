...
description: Load deep context about a topic, module, or subsystem before high-stakes work. Use when the user asks to "load into context everything about X", "remind me how X works", says "I want to know the ins and outs of X", or signals an upcoming risky change (e.g., "production breaking", "be thorough"). Drives a systematic sweep of source code, specs, related repos, postmortems, repo-level instructions (CLAUDE.md), and persistent memory — then surfaces a structured map of what was loaded so the user can correct gaps before action begins.
---
...
CLAUDE.md is the index — it tells you where this repo keeps specs, postmortems, sibling code, and conventions. Use it to guide the sweep rather than guessing paths. Beyond what CLAUDE.md points to, the standard places worth checking:
